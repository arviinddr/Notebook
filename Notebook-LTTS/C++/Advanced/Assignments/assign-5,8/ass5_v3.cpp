...
* data driven thread with references as argument 

   g++ ass5_v3.cpp -lpthread && ./a.out
...
std::thread t1(printID,std::ref(t1_flag),std::ref(t2_flag),1);
std::thread t2(printID,std::ref(t2_flag),std::ref(t1_flag),2);
